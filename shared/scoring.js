...
export function enrichPerformance(item) {
  const pulseScore = computePulseScore(item);
  return {
    ...item,
    pulseScore,
    scoreTone: getScoreTone(pulseScore)
  };
}
...
export function buildOverview(items) {
  ...
  const averageAiInvolvement = items.length
    ? Math.round(items.reduce((sum, item) => sum + item.aiInvolvement, 0) / items.length)
    : 0;
...
export function estimateSentimentScore(text = "") {
  ...
  const score = 60 + positiveHits * 8 - negativeHits * 10;
  return clamp(score);
}
...
export function estimateAiInvolvement(text = "", aiSignals = []) {
  ...
  const score = 28 + keywordScore + signalScore;
  return clamp(score);
}
